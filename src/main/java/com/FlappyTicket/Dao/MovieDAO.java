...
public class MovieDAO {
	...
	public Movie findById(int mid) {
		try {
			return jdbcTemplate.queryForObject("select * from movie where CID = ?", new BeanPropertyRowMapper<Movie>(Movie.class), mid);
	...
	public Movie findByMovieName(String mname) {
		try {
			return jdbcTemplate.queryForObject("select * from movie where CName = ?", new BeanPropertyRowMapper<Movie>(Movie.class), mname);
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
